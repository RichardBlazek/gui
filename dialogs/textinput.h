...
std::vector<std::string> Dialog(const std::string& title, const std::vector<InputDescription>& label_names, const std::string& confirm, uint32 input_size=250)
{
	...
	SDL::Window screen(title, SDL::Rect(SDL::Window::CenteredPos, labels_entries[0].second.GetPosition().x+input_size+30, labels_entries.back().second.GetPosition().Down()+buttonsize.y+15), SDL::Window::Flags::InputGrabbed);
