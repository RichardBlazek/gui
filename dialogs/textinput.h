...
std::vector<std::string> Dialog(Init& g, const std::string& title, const std::vector<InputDescription>& label_names, const std::string& confirm, uint32 input_size=250)
{
	...
	SDL::Window screen(title, SDL::Rect(SDL::Window::CenteredPos, labels_entries.second[0]->GetPosition().x+input_size+30, labels_entries.second.back()->GetPosition().Down()+buttonsize.y+15));
